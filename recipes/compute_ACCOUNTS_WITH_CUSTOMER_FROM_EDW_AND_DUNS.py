...
df = apply_rule(df, "RULE 000", [' MMCCARTHY TIRE','MCCARTHY TIRE'], 'MCCARTHY TIRE')
df = apply_rule(df, "RULE 000", ['CSC SERVICEWORKS','CSC SERVICEWORKS 0R53'], 'CSC SERVICEWORKS')
df = apply_rule(df, "RULE 000", ['CROWN CASTLE USA (0AK7)','CROWN CASTLE USA INC 0AK7'], 'CROWN CASTLE USA INC')
df = apply_rule(df, "RULE 000", ['E JOHNSON CONTROLS SECURITY','E JOHNSON CONTROLS FIRE & SEC'], 'E JOHNSON CONTROLS FIRE & SEC')
df = apply_rule(df, "RULE 000", ['FASTENAL (0469)(3)','FASTENAL (0469)(4)','FASTENAL (0469)','FASTENAL (0469)(2)','FASTENAL COMPANY'], 'FASTENAL COMPANY')
df = apply_rule(df, "RULE 000", ['FOSS NATIONAL/CORP-RATEFOSS NATIONAL LEASING (2)','FOSS NATIONAL LEASING (2)'], 'FOSS NATIONAL LEASING')
df = apply_rule(df, "RULE 000", ['GENERAL MILLS','GENERAL MILLS (3GM0)','GENERAL MILLS SALES','GENERAL MILLS OPERATIONS  3GMO'], 'GENERAL MILLS')
df = apply_rule(df, "RULE 000", ['HELMERICH AND PAYNE INC PARENT','HELMERICH  PAYNE ID 5FM8'], 'HELMERICH AND PAYNE INC')
df = apply_rule(df, "RULE 000", ['HUSSMANN CORPORATION','HUSSMAN CORPORATION'],'HUSSMAN CORPORATION')
df = apply_rule(df, "RULE 000", ['IGT GLOBAL SOLUTIONS CORP','IGT GLOBAL SOLUTIONS 0DK1'], '')
df = apply_rule(df, "RULE 000", ['J R SIMPLOT -S40','J R SIMPLOT -S40 (2)','J R SIMPLOT CO'], 'J R SIMPLOT CO')
df = apply_rule(df, "RULE 000", ['JC EHRLICH (4)','JC EHRLICH (5)','JC EHRLICH (2)'], 'JC EHRLICH')
df = apply_rule(df, "RULE 000", ['JOHNSONJOHNSON','JOHNSON JOHNSON CITRUS'], 'JOHNSON JOHNSON')
df = apply_rule(df, "RULE 000", ['KINDER MORGAN (0469)(3)','KINDER MORGAN INC','KINDER MORGAN 5GU5','KINDER MORGAN INC'], 'KINDER MORGAN INC')

df = apply_rule(df, "RULE 000", ["L'OREAL USA 3","L'OREAL USA","L'OREAL USA 2","L'OREAL CANADA INC."], "L'OREAL USA")
df = apply_rule(df, "RULE 000", ['LABCORP','LABCORP (3LAB)','LABCORP (3LAB)(2)','LABORATORY CORPORATION OF AMERICA'], 'LABORATORY CORPORATION OF AMERICA')
df = apply_rule(df, "RULE 000", ['LEHIGH HANSON (3LHN)','LEHIGH HANSON (3LHN)(2)','3995 LEHIGH HANSON INC','116710-LEHIGH HANSON, INC (2)',
...
df = apply_rule(df, "RULE 000", ['LIBERTY MUTUAL 2D93','LIBERTY MUTUAL GROUP'], 'LIBERTY MUTUAL GROUP')
df = apply_rule(df, "RULE 000", ['MARATHON PETROLEUM CORPORATION','MARATHON PETROLEUM COMPANY LP','MARATHON PETROLEUMEQUIP'], 'MARATHON PETROLEUM')
df = apply_rule(df, "RULE 000", ['MONDELEZ GLOBAL LLC','MONDELEZ GLOBAL LLC 2'], 'MONDELEZ GLOBAL LLC')
df = apply_rule(df, "RULE 000", ['NATIONAL FUEL (2G35)','NATIONAL FUEL GAS 5AP6'], 'NATIONAL FUEL')

df = apply_rule(df, "RULE 000", ['NATIONAL FUEL GAS 5AP6','NATIONAL FUEL AND LUBRICANT IN','NATIONAL FUEL (2G35)','NATIONAL FUEL'], 'NATIONAL FUEL')
df = apply_rule(df, "RULE 000", ['NEXSTAR BROADCASTING (5R10)','NEXSTAR BROADCASTING 5R10'], 'NEXSTAR BROADCASTING')

df = apply_rule(df, "RULE 000", ['NORFOLK SOUTHERN RAILWAY CO (3','NORFOLK SOUTHERN 5K16','NORFOLK SOUTHERN 5K162'], 'NORFOLK SOUTHERN')
df = apply_rule(df, "RULE 000", ['NORTHERN CLEARING INC 3','NORTHERN CLEARING INC 4'], 'NORTHERN CLEARING INC')
df = apply_rule(df, "RULE 000", ['NOVONORDISK','NOVO NORDISK','NOVO NORDISK INC','NOVO NORDISK FONDEN'], 'NOVO NORDISK INC')
df = apply_rule(df, "RULE 000", ['PHILLIPS 66 COMPANY (2J56)','PHILLIPS 66 COMPANY 2J56'], 'PHILLIPS 66 COMPANY')
df = apply_rule(df, "RULE 000", ['SCHINDLER ELEVATOR (69)','SCHINDLER ELEVATOR 5G70'], 'SCHINDLER ELEVATOR')
df = apply_rule(df, "RULE 000", ['SCI MANAGEMENT','SCI MANAGEMENT (2)'], 'SCI MANAGEMENT')
df = apply_rule(df, "RULE 000", ['SERVICE EXPERTS','SERVICE EXPERTS LD'], 'SERVICE EXPERTS')
df = apply_rule(df, "RULE 000", ['STONEMOR INC 0DS4','STONEMOR PARTNERS'], 'STONEMOR Inc')
df = apply_rule(df, "RULE 000", ['SYNGENTA (WHEELS)(2)','SYNGENTA (WHEELS)','SYNGENTA US HOLDING INC'], 'SYNGENTA')
df = apply_rule(df, "RULE 000", ['TRANSDEV (0R64)','TRANSDEV NORTH AMERI 0R64'], 'TRANSDEV')
df = apply_rule(df, "RULE 000", ['UNITED RENTALS (ARI)','UNITED RENTALS 5T55','UNITED RENTALS INC'], 'UNITED RENTALS INC')
df = apply_rule(df, "RULE 000", ['US LBM (5EE8)','US LBM (5EE8)(2)','US LBM HOLDINGS, LLC (5EE8)','LAMPERT YARDS US LBM LLC','US LBM HOLDINGS LLC 5EE8'], 'US LBM HOLDINGS LLC')
df = apply_rule(df, "RULE 000", ['VAN POOL TRANSPORTATION LLC','VAN POOL TRANSPORTATION LLC 3','VAN POOL TRANSPORTATION LLC 4','VAN POOL TRANSPORTATION'], 'VAN POOL TRANSPORTATION')
df = apply_rule(df, "RULE 000", ['VEOLIA WATER LOGISTICS (2R63)','VEOLIA LOGISTICS 2R63'], 'VEOLIA LOGISTICS')
df = apply_rule(df, "RULE 000", ['WILLIAMS STRATEGIC (0AX6)','WILLIAMS STRATEGIC (0AX6)(2)','WILLIAMS STRATEGIC 5DB0'], 'WILLIAMS STRATEGIC')
df = apply_rule(df, "RULE 000", ['XTO ENERGY (2M33)','XTO ENERGY (2M33)(1)','XTO ENERGY (2M33)','XTO ENERGY CANADA','XTO ENERGY INC 2M33'], 'XTO ENERGY INC')
